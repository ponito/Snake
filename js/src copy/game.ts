...
class Snake {
    ...
    covers(x: number, y: number): boolean {
        let part: { pos: [number, number] } = this.head.tail;
        while (part != this.head) {
            if (part.pos[0] == x && part.pos[1] == y) return true;
        }
        return false;
    }
}
...
function randomTreasure(): Treasure {
    let x = floor(Math.random() * GLOBALS.grid.cells_x),
        y = floor(Math.random() * GLOBALS.grid.cells_y);
    while (GLOBALS.player.covers(x, y)) {
        x = floor(Math.random() * GLOBALS.grid.cells_x);
        y = floor(Math.random() * GLOBALS.grid.cells_y);
    };
